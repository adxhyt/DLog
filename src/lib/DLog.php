 ...
 *  
 **/
require_once(dirname(__FILE__) . '/../conf/log.conf.php');
require_once(dirname(__FILE__) . '/writer/Writer.php');
require_once(dirname(__FILE__) . '/writer/Writer_Std.php');
...
class DLog
{
    ...
    const DEFAULT_FORMAT = '%L: %{%y-%m-%d %H:%M:%S}t %{app}x * %{pid}x [logid=%l filename=%f lineno=%N uri=%U errno=%{err_no}x %{encoded_str_array}x] %{err_msg}x';
    ...
    **/
    ...
    public static function genLogID()
    {
        ...
        if (getenv('HTTP_X_DG_LOGID') && intval(trim(getenv('HTTP_X_DG_LOGID'))) !== 0) {
            define('LOG_ID', trim(getenv('HTTP_X_DG_LOGID')));
        } else {
            $arr = gettimeofday();
     ...
     * @param DLog_Writer $writer
     */
    private static function registerWriter(DLog_Writer $writer) {
    ...
    public static function useLogerStdOut() {
        ...
        self::registerWriter(new DLog_Writer_Std());
        $used = true; 
    }
}
